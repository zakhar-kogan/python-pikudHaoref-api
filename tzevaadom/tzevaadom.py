...
def alerts_history(language="en", get_from="today"):
    ...
    try:
        response = requests.get(
            f"https://alerts-history.oref.org.il/Shared/Ajax/GetAlarmsHistory.aspx?lang={language}&mode={options_date.index(get_from) + 1}"
        )
            ...
            try:
                ...
                if len(response.content) > 5 and response.status_code == 200:
                    ...
                    filtered_areas = list(
                        {
                            area
                            for (area) in (list_oref)
                            if (area not in self.last_data)
                            and (self.filter == [] or (area) in self.filter)
                            or (
                                list_oref.count(area) > 1
                                and self.last_data.count(area) == 1
                            )
                        }
                    )
                    filtered_areas.sort()
                    ...
                    if len(filtered_areas) == 0:
                        continue
                    threading.Thread(
                        target=self.callback, args=(__format_alerts__(filtered_areas),)
                    ).start()
